...
export function Détails() {
	const { id } = useParams();

	const { axiosBaseURL } = useAxiosUrl(true);

	const { articleList, getCategoryLabel } = useArticle();
	...
	useEffect(() => {
		articleList();
		setLoading(true);
		axiosBaseURL.get("/articles/feed/" + id).then((res) => {
			setList(res.data);
			setLoading(false);
		});
	}, []);
	...
	return (
		<>
			<Header title={"Catégorie : " + getCategoryLabel(id)} />
